...
import com.example.myfinalapp.databinding.ActivityAuthBinding
...
class AuthActivity : AppCompatActivity() {
                    ...
                    if(task.isSuccessful){
                        MyApplication.auth.currentUser?.sendEmailVerification()?.addOnCompleteListener{
                            ...
                            if (sendTask.isSuccessful){
                                Toast.makeText(baseContext,"2차 인증을 위해 이메일을 확인해주세요", Toast.LENGTH_LONG).show()
                                changeVisibility("logout")
                            }
                            else{
                                Toast.makeText(baseContext,"메일 전송을 실패했습니다", Toast.LENGTH_LONG).show()
                                changeVisibility("logout")
                            }
                        }
                    }
                    else{
                        Toast.makeText(baseContext,"회원가입 실패했습니다", Toast.LENGTH_LONG).show()
                        changeVisibility("logout")
                    }
                ...
                .addOnCompleteListener(this){ task ->
                    if(task.isSuccessful){
                        if(MyApplication.checkAuth()){
                            ...
                            Toast.makeText(baseContext,"이메일 인증을 실패했습니다",Toast.LENGTH_LONG).show()
                        }
                    }
                    else{
                        Toast.makeText(baseContext,"로그인을 실패했습니다",Toast.LENGTH_LONG).show()
                        changeVisibility("logout")
                    }
                    binding.authEmailEditView.text.clear()
                    binding.authPasswordEditView.text.clear()
    ...
    fun changeVisibility(mode: String){
        if(mode.equals("signin")){
            ...
            binding.run {
                authMainTextView.text = "로그인 또는 회원가입 해주세요."
                logoutBtn.visibility = View.GONE
